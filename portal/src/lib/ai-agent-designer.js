/**
 * AI agent designer - maps natural language to AgentCore templates (browser ESM).
 */
...
export function matchAgentTemplateFromMessage(message) {
  let best = null;
  let bestScore = 0;
...
export function designAgentFromMessage(message) {
  ...
  const explanation = templateId
    ...
    : "No exact template match - loaded Custom Agent Starter. Use feature checkboxes and drag blocks to tailor your agent.";
  ...
  return {
    ...
    suggestions: [
      ...
      "Open Agent Builder canvas to fine-tune blocks and wire Gateway tools.",
      "Preview manifest for BEDROCK_GUARDRAIL_ID env vars before export.",
    ],
    userMessage: trimmed,
  };
}

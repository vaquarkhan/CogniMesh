...
export default function PatternLibrary({ activePatternId, onApplyPattern }) {
  ...
  }, [archFilter, category, search]);

  const setArchFilterAndTop = (id) => {
    setExpandedId(null);
    setArchFilter(id);
  };

  const setCategoryAndTop = (c) => {
    setExpandedId(null);
    setCategory(c);
  };
  ...
  return (
    <div className="pattern-library" ref={rootRef}>
      <div className="pattern-library-sticky">
        ...
        <div className="pattern-arch-filters">
          ...
          <button type="button" className={archFilter === "all" ? "active" : ""} onClick={() => setArchFilterAndTop("all")}>
            All
          </button>
          {ARCHITECTURE_TYPES.map((a) => (
            <button
              ...
              onClick={() => setArchFilterAndTop(a.id)}
              title={a.desc}
            >
              {a.icon} {a.label}
            </button>

# dbt Silver → Gold (+ PVDM)

<p align="center">
  <img src="../../assets/portal-canvas-datamesh.png" alt="dbt Silver → Gold (+ PVDM) - CogniMesh canvas" width="720" />
  <br /><em>dbt models · CogniMesh proof gate</em>
</p>

[← All tutorials](../README.md) · [Portal UI](../../PORTAL_UI.md)

---

## What you'll create

Use dbt for SQL modeling and schema tests, then publish through CogniMesh so Iceberg commit still requires VRP PASS. A green dbt test is observational - not a keyed multiset proof.

**Real-world example:** dbt builds silver.orders and gold.orders_daily → CogniMesh verifies sink bytes → marketplace trust card.

| | |
|---|---|
| **Pattern ID** | `dbt-silver-gold` |
| **Category** | ETL / ELT |
| **Difficulty** | Intermediate |
| **Architecture** | elt |

## Why use this pattern

Analytics engineers already live in dbt, but stewards need proof-gated lakehouse publication and marketplace trust.

## How it works

<p align="center">
  <a href="../../assets/cognimesh-pipeline-demo.mp4">
    <img src="../../assets/cognimesh-pipeline-demo-poster.png" alt="CogniMesh pipeline walkthrough: load a pattern, AWS Design Review, preview YAML, deploy, marketplace" width="720" />
  </a>
  <br /><em>Load a pattern, AWS review, preview YAML, deploy, marketplace (click to play video)</em>
</p>

```
Source → dbt silver/gold → Integrity Gate → Iceberg → Marketplace
```



**AWS services:** `dbt` · `Spark / Athena / Redshift` · `S3` · `Iceberg` · `Glue Catalog`


---

## Step-by-step in CogniMesh

### 1. Start the portal

```bash
npm run start:dev
```

Open [http://localhost:3000](http://localhost:3000).

### 2. Load this pattern

**Option A - AI Builder (recommended)**

1. Sidebar → **AI Builder** → **Data pipeline**
2. Paste: _"dbt Silver → Gold (+ PVDM) - dbt models · CogniMesh proof gate"_
3. Click **Preview pipeline plan** - read _what we'll create_ and _how it works_
4. Click **Load pipeline on canvas**

**Option B - Architectures library**

1. Sidebar → **Architectures**
2. Filter: **ETL / ELT**
3. Find **dbt Silver → Gold (+ PVDM)** → **Use pattern**

### 3. Customize blocks

Click each block on the canvas and set real values in the properties panel.

### 4. Preview & validate

Click **Preview YAML** (Ctrl+S) - review `DataContract.yaml` and Step Functions ASL.

### 5. Deploy

**Deploy** when API is on port 4000 - integrity gate → catalog registration.

---

## Developer workflow

| Layer | What you do |
|-------|-------------|
| **Portal / contract** | Tune block properties; export YAML from preview |
| **`lib/contract-builder/`** | Graph → DataContract mapping |
| **`services/pipeline-engine/`** | Contract → Step Functions ASL |
| **`lib/integrity-gate/`** | PVDM / VRP rules before gold publish |
| **`infra/terraform/`** | AWS infrastructure modules |

**API:** `POST /api/v1/pipelines/preview` · `POST /api/v1/pipelines/deploy`

---

## Tips

- Export dbt project from AWS Design Review.
- Run dbt run && dbt test, then deploy via CogniMesh for VRP-gated publish.
- Do not treat dbt test green as marketplace trust - require VRP PASS.


## Related

- [Tutorial hub](../README.md)
- [Drag-and-drop E2E](../../drag-drop-pipeline-flow.md)
- [Vaquar Pattern](../../vaquar-pattern.md)


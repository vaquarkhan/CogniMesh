# Clickstream → Real-Time Dashboards

<p align="center">
  <img src="../../assets/portal-canvas-datamesh.png" alt="Clickstream → Real-Time Dashboards - CogniMesh canvas" width="720" />
  <br /><em>Retail · Kinesis · sub-minute latency</em>
</p>

[← All tutorials](../README.md) · [Portal UI](../../PORTAL_UI.md)

---

## What you'll create

Capture website click events from Kafka, sessionize in stream mode, aggregate to gold Iceberg for near-real-time retail analytics.

**Real-world example:** Shopify-style events topic → parse page_view, add_to_cart → gold sessions table refreshed every minute.

| | |
|---|---|
| **Pattern ID** | `retail-clickstream` |
| **Category** | Retail |
| **Difficulty** | Advanced |
| **Architecture** | streaming |

## Why use this pattern

E-commerce clickstream, cart abandonment, product funnel analysis.

## How it works

```
Kafka clickstream.raw → Stream transform → Gold Iceberg sessions
```



**AWS services:** `MSK` · `Glue Streaming` · `Iceberg` · `Step Functions`


---

## Step-by-step in CogniMesh

### 1. Start the portal

```bash
npm run start:dev
```

Open [http://localhost:3000](http://localhost:3000).

### 2. Load this pattern

**Option A - AI Builder (recommended)**

1. Sidebar → **AI Builder** → **Data pipeline**
2. Paste: _"Clickstream → Real-Time Dashboards - Retail · Kinesis · sub-minute latency"_
3. Click **Preview pipeline plan** - read _what we'll create_ and _how it works_
4. Click **Load pipeline on canvas**

**Option B - Architectures library**

1. Sidebar → **Architectures**
2. Filter: **Retail**
3. Find **Clickstream → Real-Time Dashboards** → **Use pattern**

### 3. Customize blocks

Click each block on the canvas and set real values in the properties panel.

### 4. Preview & validate

Click **Preview YAML** (Ctrl+S) - review `DataContract.yaml` and Step Functions ASL.

### 5. Deploy

**Deploy** when API is on port 4000 - integrity gate → catalog registration.

---

## Developer workflow

| Layer | What you do |
|-------|-------------|
| **Portal / contract** | Tune block properties; export YAML from preview |
| **`lib/contract-builder/`** | Graph → DataContract mapping |
| **`services/pipeline-engine/`** | Contract → Step Functions ASL |
| **`lib/integrity-gate/`** | PVDM / VRP rules before gold publish |
| **`infra/terraform/`** | AWS infrastructure modules |

**API:** `POST /api/v1/pipelines/preview` · `POST /api/v1/pipelines/deploy`

---

## Tips

- Use stream execution mode.
- Partition gold table by event_date.


## Related

- [Tutorial hub](../README.md)
- [Drag-and-drop E2E](../../drag-drop-pipeline-flow.md)
- [Vaquar Pattern](../../vaquar-pattern.md)


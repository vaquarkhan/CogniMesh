# CogniMesh Data Steward

<p align="center">
  <img src="../../images/portal-agent-builder-canvas.png" alt="CogniMesh Data Steward - Agent Builder canvas" width="720" />
  <br /><em>MCP · marketplace · LF grants</em>
</p>

[← All tutorials](../README.md) · [Agent Builder guide](../../AGENT_BUILDER.md)

---

## What you'll create

Mesh steward agent for schema review, access request triage, and Lake Formation grant recommendations via CogniMesh MCP.

| | |
|---|---|
| **Template ID** | `cognimesh-steward` |
| **Category** | CogniMesh |
| **Framework** | strands |
| **Agent name** | `mesh-steward-agent` |

## Why use this agent

Automate steward workflows in CogniMesh consumer mesh.

## How it works

1. User message → **AgentCore Runtime** (session-isolated)
2. **Bedrock model** reasons over context
3. **Knowledge Base** retrieval (if enabled) augments the prompt
4. **Guardrails** filter input/output
5. **Gateway** routes tool calls to Lambda / MCP / REST
6. Response returned with observability traces

**Features:** Guardrails, Knowledge base, Gateway & tools, Observability, Human-in-the-loop

**AWS services:** `AgentCore Runtime` · `MCP` · `Guardrails` · `Lake Formation`


---

## Step-by-step in CogniMesh

### 1. Start the portal

```bash
npm run start:dev
```

### 2. Generate this agent

1. **AI Builder → AI agent** (or **Agent Builder → Templates**)
2. Enable: Guardrails, Knowledge base, Gateway & tools, Observability, Human-in-the-loop
3. Paste: _"CogniMesh data steward for access requests and Lake Formation grants"_
4. **Preview agent plan** → **Open in Agent Builder**

### 3. Customize · 4. Preview manifest · 5. Export

Edit guardrail IDs, tool names, KB IDs on canvas → **Preview manifest** → **Export manifest** (downloads YAML). AWS Bedrock provisioning is not wired in CogniMesh yet - use the manifest with `aws bedrock-agent create-agent` or Terraform.

---

## Developer workflow

| Layer | Path |
|-------|------|
| Manifest export | `portal/src/lib/agent-export.js` |
| Validation | `portal/src/lib/validate-agent-blocks.js` |
| MCP server | `services/agent-mcp/` |
| Cognitive runtime | `services/cognitive-runtime/` |

---

## Tips

- MCP tools connect to CogniMesh agent-mcp service.
- Human loop required before LF grant simulation.
- Guardrail blocks auto-approval of sensitive domains.


## Related

- [Tutorial hub](../README.md)
- [Agent Builder guide](../../AGENT_BUILDER.md)

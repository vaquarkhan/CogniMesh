 ...
 */
const { resolveBucketName } = require("../aws/resource-arns");
...
function contractToMesh(contract, options = {}) {
  ...
  const accounts = spec.governance?.accounts || options.accounts || {
    producer: options.producerAccount || process.env.AWS_ACCOUNT_ID || "ACCOUNT",
    steward: options.stewardAccount || process.env.AWS_ACCOUNT_ID || "ACCOUNT",
    publisher: options.publisherAccount || process.env.AWS_ACCOUNT_ID || "ACCOUNT",
  };
  ...
  return {
    ...
    spec: {
      ...
      runtime: {
        ...
        iceguard: {
          checkpoint_bucket:
            options.checkpointBucket ||
            resolveBucketName("CHECKPOINT_BUCKET_NAME", null) ||
            spec.target?.location?.replace(/\/[^/]*$/, "/checkpoints") ||
            null,
          proof_bucket:
            options.proofBucket ||
            resolveBucketName("PROOF_BUCKET_NAME", null) ||
            null,
          lakehouse_bucket: spec.target.location || options.lakehouseBucket || null,
        },
        durable: {

  ...
  return {
    ...
    States: {
      ...
      RouteOutcome: {
        Type: "Choice",
        Choices: [
          { Variable: "$.result.Payload.outcome", StringEquals: "committed", Next: "Success" },
          { Variable: "$.result.Payload.outcome", StringEquals: "resumed", Next: "Success" },
          { Variable: "$.result.Payload.outcome", StringEquals: "rolled_back", Next: "IncrementResumeAttempt" },
          { Variable: "$.result.Payload.outcome", StringEquals: "verification_failed", Next: "VerificationFailed" },
          { Variable: "$.result.Payload.outcome", StringEquals: "signing_failed", Next: "VerificationFailed" },
          { Variable: "$.result.Payload.outcome", StringEquals: "publish_blocked", Next: "VerificationFailed" },
          { Variable: "$.result.Payload.outcome", StringEquals: "unverified", Next: "Unverified" },
        ],
        Default: "UnknownFailure",
      },
      /**
       * Advance resume_offset from IceGuard rollback payload so the next Lambda
       * invoke continues after the last committed chunk (not from zero).
       */
      IncrementResumeAttempt: {
        ...
        Parameters: {
          workload: {
            contract: contract,
            "source_rows.$": "$.workload.source_rows",
            "workload_id.$": "$.result.Payload.workload_id",
            "resume_offset.$": "$.result.Payload.resume_offset",
          },
      ...
      Unverified: {
        Type: "Fail",
        Error: "Unverified",
        Cause: "PVDM did not run a verifiable workload (empty or filtered). No snapshot was published.",
      },
      VerificationFailed: {
        Type: "Fail",

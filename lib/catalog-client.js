...
/** API-only dev (`CATALOG_STORAGE=memory`): use in-process store, not remote :8080. */
function embeddedPrimary() {
  return String(process.env.CATALOG_STORAGE || "").toLowerCase() === "memory";
}
...
function patchProductTrust(id, trust) {
  const record = embedded.get(id);
  if (!record) return null;
  record.trust = trust;
  record.tags = {
    ...(record.tags || {}),
    proofGated: trust.proofGated ? "true" : "false",
    trustGrade: trust.grade,
    conformanceProfile: trust.conformanceProfile || "",
    icebergSnapshotId: trust.icebergSnapshotId || "",
    sourceSnapshotId: trust.sourceSnapshotId || "",
    lastProofAt: trust.lastProofAt || "",
    vrpVerdict: trust.vrpVerdict || "",
  };
  embedded.set(id, record);
  return toResponse(record);
}
...
async function registerProduct(payload, auth = {}) {
  if (embeddedPrimary()) {
    return { source: "embedded", product: registerEmbedded(payload) };
  }
  try {

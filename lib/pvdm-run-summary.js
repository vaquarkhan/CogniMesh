...
async function buildPvdmRunSummary(contract, options = {}) {
  ...
  let conformanceProfile = null;
  let sourceSnapshotId = null;
  let schemaFingerprint = null;
  let profileTCertified = false;
  ...
  try {
    if (contract.spec?.transform?.pvdm && options.runLive !== false) {
      ...
      if (result.proof) {
        conformanceProfile = result.proof.conformance_profile || null;
        sourceSnapshotId = result.proof.source_snapshot_id || null;
        schemaFingerprint = result.proof.schema_fingerprint || null;
        profileTCertified = Boolean(result.proof.profile_t_certified);
      }

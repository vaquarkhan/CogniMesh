...
async function deployStateMachine(contract, stateMachineDefinition, options = {}) {
  ...
  if (process.env.AWS_DEPLOY_EXECUTE === "true") {
    // Start the execution with schema-aligned sample rows so the domain-writer has real data
    const pvdm = contract?.spec?.transform?.pvdm || {};
    const fields = [
      ...(Array.isArray(pvdm.identityFields) ? pvdm.identityFields : []),
      ...(Array.isArray(pvdm.contentFields) ? pvdm.contentFields : []),
    ];
    const uniqueFields = [...new Set(fields)];
    const mkRow = (seed) => {
      if (!uniqueFields.length) return { id: String(seed), value: seed * 10 };
      const row = {};
      for (const f of uniqueFields) row[f] = `${f}-${seed}`;
      return row;
    };
    const sampleRows = [mkRow(1), mkRow(2), mkRow(3)];
    ...
    const started = await client.send(new StartExecutionCommand({
      ...
      input: JSON.stringify({
        ...
        workload_id: `deploy-${contract.metadata.name}-${Date.now()}`,
      }),
    }));
    execution = {

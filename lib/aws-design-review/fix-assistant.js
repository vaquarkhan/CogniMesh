...
function resolvePlaybook(findingId) {
  ...
  if (findingId.startsWith("sec.integrity.")) {
    return {
      fields: [],
      steps: [
        "Click Apply fix - we'll update the affected block on your canvas.",
        "If the issue is a missing Secrets Manager ARN, switch to Create new database in Properties.",
        "Re-run Preview when done. No YAML editing required.",
      ],
    };
  }
  return null;

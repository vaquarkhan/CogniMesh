...
const PLAYBOOK = {
  "sec.secrets_manager": {
    fields: ["secretArn"],
    steps: [
      "Select the RDS/MySQL source block on the canvas (or click Focus block below).",
      "Open the Properties panel on the right.",
      "Set Credentials → secretArn to your Secrets Manager ARN (never paste raw passwords).",
  ...
  "sec.rds_private": {
    fields: ["vpcSecurityGroup", "privateSubnet"],
    steps: [
      "Ensure RDS is in private subnets (no public accessibility).",
      "Set vpcSecurityGroup to a security group that only allows Glue/DMS from your VPC.",
      "Use a Glue VPC connection or DMS replication instance in the same VPC for ingest.",
    ],
    suggestPatch: () => ({
      vpcSecurityGroup: "sg-xxxxxxxx",
      privateSubnet: true,
    }),
  },
  "sec.tls_transit": {
...
function buildFixPlan(finding, nodes, pipelineMeta) {
  ...
  const propertyPatch =
    playbook?.suggestPatch && node ? playbook.suggestPatch(node, pipelineMeta) : null;

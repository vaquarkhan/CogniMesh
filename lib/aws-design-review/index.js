  ...
  for (const n of nodes) {
    ...
    if (bt === "source") {
      ...
      if (st === "rds" || st === "mysql") {
        ...
        const credsOk = mode === "provision" || Boolean(d.secretArn);
        addService("rds", "rds", mode === "provision" ? "RDS (Terraform)" : "Amazon RDS", credsOk ? "ok" : "issue");
        addService("secrets", "secrets", "Secrets Manager", credsOk ? "ok" : "issue");
        link("vpc", "rds");
        link("rds", "secrets");
        link("secrets", "glue");
      } else if (st === "s3") {
        ...
        addService("kinesis", "kinesis", "Kinesis Data Streams", "ok");
        link("kinesis", "glue-etl");
      }
    }

...
function runSecurityReview({ nodes, edges, pipelineMeta, contract, integrityGate }) {
  ...
  for (const src of sources) {
    ...
    if (st === "rds" || st === "mysql") {
      const mode = d.rdsProvisioningMode === "provision" ? "provision" : "existing";
      if (mode === "provision") {
        if (!d.database || !d.table) {
          findings.push(
            finding({
              id: `arch.rds_schema.${src.id}`,
              severity: "medium",
              pillar: "Operational Excellence",
              title: "RDS provision: set database and table",
              message: `Source "${d.label || src.id}" will be created via Terraform — specify database and table names for the contract.`,
              fix: "Set database and table in Properties. Export infrastructure Terraform from AWS Design Review.",
              nodeIds: [src.id],
        ...
        if (!secretArn || !SECRETS_ARN.test(secretArn)) {
          findings.push(
            finding({
              ...
              message: `Source "${d.label || src.id}" (${st}) uses an existing database — credentials must reference Secrets Manager (not inline passwords).`,
              fix: "Set secretArn, or switch RDS database → Create new (Terraform) to provision RDS + secret automatically.",
              nodeIds: [src.id],
        ...
        if (!d.vpcSecurityGroup && !d.privateSubnet) {
          findings.push(
            finding({
              ...
              message: "Existing production RDS endpoints must not be publicly reachable.",
              fix: "Deploy RDS in private subnets with VPC security groups, or switch to Create new (Terraform).",
              nodeIds: [src.id],

...
const GLUE_IDENTIFIER = /^[A-Za-z0-9_]+$/;

function assertGlueIdentifier(name, label) {
  if (typeof name !== "string" || !GLUE_IDENTIFIER.test(name)) {
...
function athenaConsoleUrl({ database, table, region, workgroup } = {}) {
  assertGlueIdentifier(database, "database");
  assertGlueIdentifier(table, "table");
...
module.exports = { athenaConsoleUrl, parseSchemaFromManifest, sampleRowsFromSchema, assertGlueIdentifier };

...
function mountMarketplaceRoutes(app, { requireAuth }) {
  ...
  app.post("/api/v1/marketplace/products/:id/subscription-token", requireAuth, async (req, res) => {
    const span = startSpan("api.marketplace.subscription_token", {
      user_id: req.auth?.sub,
      product_id: req.params.id,
    });
    const result = await issueSubscriptionToken(req.params.id, req.auth || {}, req.body || {});
    span.end(result.status === "success" ? "ok" : "error", { code: result.code || "" });
    const status =
      result.status === "success" ? 201 : result.code === "MARKETPLACE_PRODUCT_NOT_FOUND" ? 404 : 403;
    res.status(status).json(result);
  });

  app.post("/api/v1/marketplace/products/:id/schema-diff", requireAuth, async (req, res) => {
    const span = startSpan("api.marketplace.schema_diff", {
      user_id: req.auth?.sub,
      product_id: req.params.id,
    });
    const result = await schemaDiffProduct(req.params.id, req.auth || {}, req.body || {});
    span.end(result.status === "success" ? "ok" : "error", { code: result.code || "" });
  ...
  app.post("/api/v1/marketplace/sla", requireAuth, (req, res) => {
    ...
    if (!body.productId) {
      return res.status(400).json({
        ...
        fixHint: "POST { productId, consumerId?, slaMinutes?, penalty?, webhookUrl?, channel? }",
      });
    }
  ...
  app.get("/api/v1/marketplace/sla/check", requireAuth, async (req, res) => {
    ...
    let lastRunAt = req.query.lastRunAt || null;
    let domain = null;
    let productName = null;
    if (!lastRunAt) {
      try {
        const fetched = await getProduct(req.query.productId, req.auth || {});
        const product = fetched?.product || fetched;
        lastRunAt =
          product?.trust?.lastProofAt ||
          product?.tags?.lastProofAt ||
    ...
    const notify = String(req.query.notify || "").toLowerCase() === "true";
    if (notify) {

...
const { getAwsAccountId } = require("../aws/account-id");
...
async function deployPipeline({ nodes, edges, pipelineMeta, catalogUrl, auth }) {
  ...
  const deployRegion =
    contract.metadata?.awsRegion || contract.spec?.deployment?.region || process.env.AWS_REGION || "us-east-1";
  const awsResourceOpts = {
    accountId,
    producerAccount: accountId || undefined,
    stewardAccount: accountId || undefined,
    publisherAccount: accountId || undefined,
    region: deployRegion,
    namePrefix: process.env.AWS_NAME_PREFIX || `cognimesh-${contract.metadata.domain}`,
    checkpointBucket: process.env.CHECKPOINT_BUCKET_NAME,
    proofBucket: process.env.PROOF_BUCKET_NAME,
    domain: contract.metadata.domain,
  };
  ...
  const compiled = compileContractSmart(contract, { aws: awsResourceOpts });
  let stateMachine = compiled.stateMachine;
  const vaquar = compiled.vaquar || null;

  let graphStateMachine = null;
  if (isWorkflowGraph(nodes)) {
    const wf = compileGraphToStateMachine(nodes, edges, {
      name: contract.metadata.name,
      namePrefix: process.env.AWS_NAME_PREFIX || `cognimesh-${contract.metadata.domain}`,
    });
    if (wf.success) graphStateMachine = wf.stateMachine;
  }

  let aws = { deployed: false, reason: "Not attempted" };
  try {
    aws = await deployStateMachine(contract, stateMachine, { region: deployRegion });
  } catch (err) {
  ...
  const realAwsDeploy = Boolean(aws.deployed);
  const pvdmSummary = await buildPvdmRunSummary(contract, { runLive: !realAwsDeploy });
  if (realAwsDeploy && pvdmSummary.vrpVerdict === "UNVERIFIED") {
    pvdmSummary.message = "Deployed to AWS Step Functions. VRP proof is generated when the pipeline runs on real data.";
    pvdmSummary.pvdmOutcome = "deployed";
  }

  if (catalogProduct?.id) {
    const { productTrust } = require("../vrp/product-trust");
    const trust = productTrust({
      proofGated: pvdmSummary.proofGated,
      vrpVerdict: pvdmSummary.vrpVerdict,
      conformanceProfile: pvdmSummary.conformanceProfile,
      profileTCertified: pvdmSummary.profileTCertified,
      icebergSnapshotId: pvdmSummary.icebergSnapshotId,
      sourceSnapshotId: pvdmSummary.sourceSnapshotId,
      schemaFingerprint: pvdmSummary.schemaFingerprint,
      lastProofAt: new Date().toISOString(),
    });
    pvdmSummary.trust = trust;
    const patched = patchProductTrust(catalogProduct.id, trust);
    if (patched) catalogProduct = patched;
  }

  const proofBlocked =
    pvdmSummary.signingFailed ||
    (pvdmSummary.proofGated && pvdmSummary.vrpVerdict === "FAIL");

  const runOutcome =
    pvdmSummary.vrpVerdict === "PASS"
      ? "success"
      : pvdmSummary.vrpVerdict === "FAIL"
        ? "verification_failed"
        : realAwsDeploy
          ? "deployed"
          : "unverified";
  ...
  return {
    status: proofBlocked ? "error" : "success",
    stage: proofBlocked ? (pvdmSummary.signingFailed ? "vrp_signing" : "vrp_verification") : undefined,
    contract,
    manifestYaml,
    stateMachine,
    graphStateMachine,
  ...
  try {
    if (isWorkflowGraph(nodes)) {
      ...
      const previewAws = {
        ...
        producerAccount: process.env.AWS_ACCOUNT_ID,
      ...
      const compiled = compileContractSmart(contract, { aws: previewAws });

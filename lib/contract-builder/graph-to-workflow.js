 ...
 * Uses pluggable compute-engine registry for task generation.
 */

const { resolveEngine } = require("./compute-engines");
...
function taskStateForNode(node, prefix) {
  ...
  switch (node.data.blockType) {
    ...
    case "source":
      return resolveEngine(node).task("source", node, prefix);
    case "transform":
      ...
      return resolveEngine(node).task("transform", node, prefix);
    case "sink":
      return resolveEngine(node).task("sink", node, prefix);
    case "merge":
...
/**
 * Find the node where parallel branches reconverge.
 */
function findConvergenceNode(nodes, edges, branchStartIds) {
  if (!branchStartIds.length) return null;
  const { outgoing } = buildAdjacency(nodes, edges);
  const reachSets = branchStartIds.map((startId) => {
    const seen = new Set();
    const stack = [startId];
    while (stack.length) {
      const id = stack.pop();
  ...
  const common = [...reachSets[0]].filter((id) => reachSets.every((s) => s.has(id)));
  if (!common.length) return null;
  const mergeBlocks = common.filter((id) => nodes.find((n) => n.id === id)?.data.blockType === "merge");
  return mergeBlocks[0] || common[0];
}

/** Mark any state with neither Next nor End (and not routing/terminal) as End. */
function finalizeTerminals(states) {
  for (const s of Object.values(states)) {
    if (!s || typeof s !== "object") continue;
    if (s.Type === "Choice" || s.Type === "Succeed" || s.Type === "Fail") continue;
    if (!s.Next && !s.End) s.End = true;
  }
  return states;
}

/** Collect every state name referenced via Next / Choice targets / Default. */
function collectReferencedTargets(states) {
  const refs = new Set();
  for (const s of Object.values(states)) {
    if (!s || typeof s !== "object") continue;
    if (s.Next) refs.add(s.Next);
    if (s.Type === "Choice") {
      for (const c of s.Choices || []) if (c.Next) refs.add(c.Next);
      if (s.Default) refs.add(s.Default);
    }
  }
  return refs;
}
...
/**
 * Materialize referenced-but-undefined targets into the states map.
 */
function completeReferencedTargets(nodes, edges, states, prefix) {
  let guard = 0;
  while (guard++ < nodes.length + 5) {
    const missing = [...collectReferencedTargets(states)].filter((name) => !states[name]);
    if (!missing.length) break;
    let progressed = false;
    for (const missingName of missing) {
      const node = nodes.find((n) => stateNameFor(n) === missingName);
      if (node && node.data.blockType !== "choice" && node.data.blockType !== "parallel") {
        const before = Object.keys(states).length;
        linearChainStates(nodes, edges, node.id, null, prefix, states);
        if (Object.keys(states).length > before) progressed = true;
      }
      if (!states[missingName]) {
        states[missingName] = taskStateForNode(node || { data: { blockType: "pass" } }, prefix);
        states[missingName].End = true;
        progressed = true;
      }
    }
    if (!progressed) break;
...
function compileGraphToStateMachine(nodes, edges, pipelineMeta = {}) {
  ...
  return {
    ...
    stateMachine: {
      Comment: `CogniMesh workflow: ${pipelineMeta.name || "pipeline"}`,
      StartAt: startAt,
      States: finalizeTerminals(completeReferencedTargets(nodes, edges, states, prefix)),
      cognimesh: {
        mode: "workflow-graph",
...
function isWorkflowGraph(nodes) {
  ...
  const blockType = (n) => n?.data?.blockType;
  const sources = nodes.filter((n) => blockType(n) === "source").length;
  const sinks = nodes.filter((n) => blockType(n) === "sink").length;
  const flows = nodes.filter((n) => FLOW_TYPES.has(blockType(n))).length;

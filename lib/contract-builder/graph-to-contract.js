...
function buildTransform(transformNode, sourceNode) {
  ...
  if (d.transformType === "spark_declarative") {
    transform.type = "spark_declarative";
    transform.sdp = {
      storage: d.sdpStorage || undefined,
      layers: {
        bronze: d.bronzeSql ? { sql: d.bronzeSql } : undefined,
        silver: d.silverSql ? { sql: d.silverSql } : undefined,
        gold: d.sparkSql ? { sql: d.sparkSql } : undefined,
      },
    };
  }
  if (d.transformType === "dbt") {
    transform.type = "dbt";
    transform.dbt = {
      profile: d.dbtProfile || "cognimesh",
      materialization: d.dbtMaterialization || "table",
      projectDir: d.dbtProjectDir || undefined,
    };
  }

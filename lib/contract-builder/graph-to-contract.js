...
function buildSource(sourceNode) {
  ...
  if (d.sourceType === "rds" || d.sourceType === "mysql") {
    ...
    source.provisioning = {
      mode: d.rdsProvisioningMode === "provision" ? "terraform" : "existing",
    };
    if (d.rdsProvisioningMode === "provision") {
      if (d.secretArn) source.connection.secretArn = d.secretArn;
    } else {
      ...
      if (d.secretArn) {
        ...
        source.connection.secretArn = `arn:aws:secretsmanager:${region}:${acct}:secret:placeholder`;
      }
    }
  }

...
function proofGatedCommit({
  ...
  if (!proof.source_snapshot_id) {
    const err = new Error("proof missing source_snapshot_id (paper N15)");
    err.code = "VERIFICATION_FAILED";
    throw err;
  }
  if (expectedSourceSnapshot && String(proof.source_snapshot_id) !== String(expectedSourceSnapshot)) {
    const err = new Error("source snapshot mismatch — cannot certify across source snapshots (paper N15)");
    err.code = "VERIFICATION_FAILED";
    throw err;
  }
  if (
    proof.source_schema_fingerprint &&
    proof.sink_schema_fingerprint &&
    proof.source_schema_fingerprint !== proof.sink_schema_fingerprint
  ) {
    const err = new Error("schema fingerprint mismatch — cannot certify across differing schema (paper N15)");
    err.code = "VERIFICATION_FAILED";
    throw err;
  }
  if (expectedSchemaFingerprint && proof.schema_fingerprint !== expectedSchemaFingerprint) {
    const err = new Error("schema fingerprint does not match expected schema (paper N15)");
    err.code = "VERIFICATION_FAILED";
    throw err;
  }

...
async function readParquetRecords(localPath) {
  ...
  try {
    ...
    // Windows can briefly lock the file after ParquetWriter.close (EINVAL/EBUSY).
    if (/EINVAL|EBUSY|EPERM|EACCES|locked/i.test(String(err.message || err))) {
      ...
      try {
        ...
        await reader.close();
        const fresh = fs.readFileSync(localPath);
        return {
          rows,
          bytes: fresh,
          sha256: fullFileSha256(fresh),
          footer_sha256: parquetFooterSha256(fresh),
          digest_type: "parquet_footer",
          format: "parquet",
        };
      } catch (retryErr) {
        throw new Error(`Parquet read-back failed: ${retryErr.message}`);
      }
    }

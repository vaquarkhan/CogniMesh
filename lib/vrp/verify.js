...
function proofBodyFromEnvelope(proof) {
  ...
  const { signing: _signing, proof_id: _proofId, commit_receipt: _receipt, ...body } = proof;
  return body;
}
...
function verifyMultisetBinding(proof) {
  ...
  if (
    mode === "identity" &&
    multiset.identity_source_hash &&
    multiset.identity_source_hash !== multiset.identity_sink_hash
  ) {
    return { valid: false, reason: "identity digest mismatch (paper §7.1 D_k^id)" };
  }
...
function verifyVrpProof(proof, options = {}) {
  ...
  const checks = {
    ...
    snapshot:
      options.requireSnapshotPin === true && !snapshotId
        ? { valid: false, reason: "missing iceberg_snapshot_id" }
        : { valid: true, skipped: !snapshotId },

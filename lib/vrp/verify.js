...
function verifySinkFileDigest(proof, options = {}) {
  ...
  const match = digests.every((d) => {
    ...
    if (digestType === "parquet_footer") {
      const expected = d.footer_sha256 || d.sha256;
      return expected === footerSha256;
    }
    const expected = d.full_sha256 || d.sha256;
    return expected === fullSha256;
  });
  ...
  return {
    ...
    digest_type: digests[0]?.digest_type || "parquet_footer",
  };
}

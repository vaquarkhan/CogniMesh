...
async function serveProofGatedDataset(options = {}) {
  ...
  const pinnedSnapshot =
    proof.commit_receipt?.iceberg_snapshot_id || proof.iceberg_snapshot_id || null;
  const snapshotId = await resolveSnapshotForRead(proof.table, pinnedSnapshot);
  if (pinnedSnapshot && snapshotId && String(snapshotId) !== String(pinnedSnapshot)) {
  ...
  const stamp = {
    ...
    iceberg_snapshot_id: pinnedSnapshot,

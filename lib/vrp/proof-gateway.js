...
const { assertGatedConsumerSnapshot } = require("./consumer-snapshot");
...
/**
 * Proof-aware data gateway: verify proof, optionally enforce marketplace subscription token,
 * read pinned snapshot materialization, stamp what was served.
 */
async function serveProofGatedDataset(options = {}) {
  ...
  const requireSub =
    ...
    String(process.env.VRP_REQUIRE_SUBSCRIPTION_TOKEN || "").toLowerCase() === "true";
  if (requireSub) {
    const { verifySubscriptionToken } = require("./subscription-token");
    const subToken =
      options.subscriptionToken ||
      options.headers?.["x-cognimesh-subscription-token"] ||
      options.headers?.["X-CogniMesh-Subscription-Token"];
    const subCheck = verifySubscriptionToken(subToken, {
      productId: options.productId || undefined,
    });
    if (!subCheck.valid) {
      throw new ProofGatewayError(
        subCheck.reason || "subscription token required",
        "SUBSCRIPTION_TOKEN_DENIED"
      );
    }
    if (
      options.productId &&
      subCheck.stamp.product_id &&
      subCheck.stamp.product_id !== options.productId
    ) {
      throw new ProofGatewayError("subscription token product mismatch", "SUBSCRIPTION_TOKEN_DENIED");
    }
  }
  ...
  try {
    publishedSnapshot = assertGatedConsumerSnapshot(proof, {
      requestedSnapshotId: options.requestedSnapshotId,
    });
  } catch (err) {
    throw new ProofGatewayError(err.message, err.code || "STAGING_SNAPSHOT_DENIED");
  }
  const snapshotId = await resolveSnapshotForRead(proof.table, publishedSnapshot);
  if (publishedSnapshot && snapshotId && String(snapshotId) !== String(publishedSnapshot)) {
  ...
  const stamp = {
    ...
    iceberg_snapshot_id: publishedSnapshot,

...
function canonicalRowBytes(row, fields) {
  return Buffer.from(canonicalJson(projectRow(row, fields)), "utf8");
}
...
function hashMultiset(rows, fields, options = {}) {
  ...
  for (const row of rows || []) {
    acc = (acc + elementHash(key, canonicalRowBytes(row, fields))) % MOD;
  }

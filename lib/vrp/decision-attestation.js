...
function normalizeInputBinding(proof) {
  return {
...
async function buildDecisionAttestation(options = {}) {
  ...
  if (!inputProofs.length) {
    return { verdict: "UNVERIFIED", error: "no input proofs — attestation requires verified inputs", attestation: null };
  }

  const verifiedInputs = [];
  for (const proof of inputProofs) {
    const verification = verifyVrpProof(proof, { now, requireSignature: sign });

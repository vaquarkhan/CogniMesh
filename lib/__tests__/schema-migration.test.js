...
describe("schema migration / env", () => {
  ...
  it("checkApiVersionMigration allows v1", () => {
    const r = checkApiVersionMigration({ apiVersion: "cognimesh.io/v1" });
    assert.equal(r.allowed, true);
    assert.equal(isSupportedVersion("cognimesh.io/v1"), true);
  });

  it("checkApiVersionMigration rejects v2 with upgrade hint", () => {
    const r = checkApiVersionMigration({ apiVersion: "cognimesh.io/v2" });
    assert.equal(r.allowed, false);
    assert.match(r.errors[0], /not yet supported/i);
    assert.equal(r.upgradeHint, "v2-migration-not-available");
  });

  it("migrateV1ToV2 throws until v2 schema exists", () => {
    assert.throws(() => migrateV1ToV2({ apiVersion: "cognimesh.io/v1" }), /not yet supported/i);
  });

...
describe("SDP and dbt export", () => {
  ...
  it("exportProjectBundle returns actionable errors without a contract name", () => {
    const missing = exportProjectBundle("sdp", { metadata: {}, spec: {} });
    assert.equal(missing.status, "error");
    assert.equal(missing.code, "EXPORT_NAME_REQUIRED");
    assert.match(missing.fixHint || "", /Pipeline settings|pattern/i);

    const none = exportProjectBundle("dbt", null);
    assert.equal(none.status, "error");
    assert.equal(none.code, "EXPORT_CONTRACT_REQUIRED");
  });
});

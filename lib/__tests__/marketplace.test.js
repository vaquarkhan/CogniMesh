...
describe("marketplace API facade", () => {
  ...
  it("diffProductSchemas flags type narrowing and nullability tighten", () => {
    const diff = diffProductSchemas({
      left: [
    ...
    assert.equal(diff.breaking, true);
    assert.ok(diff.schema.typeChanges.some((t) => t.column === "amount" && t.breaking));
    assert.ok(diff.schema.nullabilityChanges.some((n) => n.column === "code" && n.breaking));
  });

  it("marketplaceCatalog publishes trust rubric", () => {
    const cat = marketplaceCatalog();
    assert.ok(cat.trustRubric);
    assert.equal(cat.trustRubric.featured.minScore, 55);
    assert.ok(cat.trustRubric.gradeBtoA.typical.includes("grade A"));
  });

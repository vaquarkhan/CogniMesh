...
describe("VRP offline verification", () => {
  ...
  it("verifyVrpProof fails when multiset hashes differ", async () => {
    ...
    pass.proof.multiset.sink_materialization = "read_back";
  ...
  it("verifyVrpProof passes structurally valid unsigned proof", async () => {
    ...
    const vrp = await generateVRP(rows, rows, {
      identityFields: ["id"],
      contentFields: ["id", "v"],
      parquetUri: "s3://bucket/f.parquet",
      sinkFileDigest: { sha256: "abc", row_count: "1" },
      icebergSnapshotId: "1",
      sign: false,
    });
    vrp.proof.multiset.sink_materialization = "read_back";
  ...
  it("verifyVrpProof validates dev-signed proof with embedded public key", async () => {
    ...
    const vrp = await generateVRP(rows, rows, {
      ...
      contentFields: ["id", "v"],
      parquetUri: "s3://bucket/f.parquet",
      sinkFileDigest: { sha256: "live", row_count: "1" },

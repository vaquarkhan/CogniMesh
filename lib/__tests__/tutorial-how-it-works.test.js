...
const DEMOS = [
  { base: "cognimesh-howto-demo", minMp4: 80_000 },
...
describe("tutorial how-it-works videos", () => {
  ...
  it("getting-started tutorial How it works links the captioned tour", () => {
    const md = read("docs/tutorials/getting-started-ui.md");
    assert.match(md, /^## How it works/m);
    assert.match(md, /cognimesh-howto-demo\.mp4/);
    assert.match(md, /cognimesh-howto-demo-poster\.png/);
  ...
  it("tutorial hub and generated pages embed how-it-works videos", () => {
    ...
    assert.match(hub, /cognimesh-howto-demo\.mp4/);
  ...
  it("demo capture script records the captioned howto plus four clips", () => {
    const src = read("scripts/capture-portal-demo.js");
    for (const { base } of DEMOS) {
      assert.match(src, new RegExp(`"${base}"`));
    }
    assert.match(src, /runHowtoDemoFlow/);
    assert.match(src, /showChapter/);

...
process.env.ICEBERG_SNAPSHOT_STATE = require("path").join(
  require("os").tmpdir(),
  `iceberg-snapshots-pvdm-${process.pid}.json`
);
...
describe("pvdm failure paths", () => {
  ...
  it("IceGuardWriter rollback clears uncommitted checkpoints", async () => {
    const w = new IceGuardWriter({ isolationId: "rollback-test" });
    await w.writeChunk(0, [{ a: 1 }], "s3://staging");
    await w.writeChunk(1, [{ a: 2 }], "s3://staging");
    w.commitChunk(0);
    const rb = w.rollback();
    assert.equal(rb.rolledBack, 1);
    assert.ok(Array.isArray(rb.deletedPaths));
  });

  it("IceGuardWriter aborts when remaining time is below threshold", async () => {
    const w = new IceGuardWriter({
      isolationId: "timeout-test",
      rollbackThresholdMs: 30000,
      getRemainingMs: () => 5000,
    });
    await assert.rejects(() => w.writeChunk(0, [{ a: 1 }], "s3://staging"), /IceGuardRollback/);
  });

  it("IceGuardWriter nextResumeOffset advances past committed chunks", () => {
    const w = new IceGuardWriter({ isolationId: "offset-test" });
    w.committedChunks.push(0, 1);
    assert.equal(w.nextResumeOffset(5000, 0), 10000);
  });

...
describe("VRP sink read-back and proof honesty", () => {
  ...
  it("sink hash uses read-back rows not shallow copy", async () => {
    ...
    assert.equal(vrp.proof.sink_artifacts.file_digests[0].digest_type, readBack.digest_type);
  });

  it("verifyVrpProof checks logical content when compaction_safe", async () => {
    const rows = [{ id: "1", v: 10 }];
    ...
    const tamperedRows = readBack.rows.map((r) => ({ ...r, v: 99 }));
    const tampered = verifyVrpProof(vrp.proof, {
      requireSignature: false,
      sinkRows: tamperedRows,
      localPath,
    });
    assert.equal(tampered.valid, false);
    assert.match(tampered.reason, /logical content/);
  });
  ...
  it("runPvdmWorkload VRP passes only after durable Parquet read-back", async () => {
    ...
    assert.equal(result.proof.iceberg_snapshot_id, null);
    assert.equal(result.proof.commit_receipt.iceberg_snapshot_id, result.snapshot_id);
    assert.match(result.proof.sink_artifacts.file_digests[0]?.digest_type || "", /parquet_footer|full_file/);
    const verified = verifyVrpProof(result.proof, { requireSignature: false });
    assert.equal(verified.valid, true, verified.reason);
  });

  it("falls back to NDJSON durable read-back when Parquet is unavailable", async () => {
    process.env.VRP_FORCE_NDJSON = "true";
    const contract = {
      metadata: { name: "ndjson-fb", domain: "d" },
      spec: {
        transform: { pvdm: { identityFields: ["id"], contentFields: ["id", "v"] } },
        target: { location: "s3://t/", catalog: { database: "d", table: "ndjson" } },
      },
    };
    const result = await runPvdmWorkload({
      contract,
      source_rows: [{ id: "1", v: 1 }],
      workload_id: "ndjson-fallback-test",
    });
    delete process.env.VRP_FORCE_NDJSON;
    assert.equal(result.outcome, "committed");
    assert.equal(result.proof.multiset.sink_materialization, "read_back");
    assert.equal(result.proof.sink_artifacts.file_digests[0]?.digest_type, "full_file");
  });

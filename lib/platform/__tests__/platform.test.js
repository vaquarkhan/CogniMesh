...
describe("platform modules", () => {
  ...
  it("platform store read/write file backend", () => {
    const tmp = path.join(os.tmpdir(), `platform-store-${Date.now()}`);
    const prev = process.env.PLATFORM_DATA_DIR;
    process.env.PLATFORM_DATA_DIR = tmp;
    process.env.PLATFORM_STORE = "file";
    const { readStore, writeStore } = require("../platform-store");
    writeStore("test-store", { hello: "world" });
    const data = readStore("test-store", null);
    assert.equal(data.hello, "world");
    process.env.PLATFORM_DATA_DIR = prev;
  });
});

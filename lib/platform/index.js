...
function mountPlatformRoutes(app, { requireAuth }) {
  ...
  app.post("/api/v1/platform/preview-source", requireAuth, async (req, res) => {
    const span = startSpan("api.preview-source", { user_id: req.auth?.sub });
    const { nodes, edges, pipelineMeta, limit } = req.body || {};
    const graph = graphToContractSmart(nodes, edges || [], pipelineMeta || {});
    if (!graph.success) {
      span.end("error", { reason: "graph_invalid" });
      return res.status(400).json({ status: "error", errors: graph.errors });
    }
    const sampleSpan = startSpan(
      "preview-source.sample",
      { user_id: req.auth?.sub, source_type: graph.contract?.spec?.source?.type },
      span
    );
    const result = await previewSourceData(graph.contract, { limit: limit || 10 });
    sampleSpan.end(result.live ? "ok" : "simulated", {
      live: Boolean(result.live),
      row_count: result.rowCount,
      simulated: Boolean(result.simulated ?? !result.live),
    });
    span.end("ok", { live: Boolean(result.live), row_count: result.rowCount });
    res.json(result);
  });

  app.get("/api/v1/agents/deploy-targets", requireAuth, (_req, res) => {
    const { runtimeEnabled } = require("./agentcore-runtime-deploy");
    res.json({
      targets: [
        { id: "bedrock-agents", label: "Bedrock Agents", default: true, liveDeploy: true },
        { id: "agentcore-runtime", label: "AgentCore Runtime (Strands)", default: false, liveDeploy: runtimeEnabled() },
      ],
    });
  });

  app.post("/api/v1/agents/deploy", requireAuth, async (req, res) => {
    const { manifest, deployTarget } = req.body || {};
    if (!manifest?.metadata?.name) {
      return res.status(400).json({ status: "error", errors: ["manifest required"] });
    }

    // AgentCore Runtime (Strands) target
    if (deployTarget === "agentcore-runtime") {
      const { deployAgentCoreRuntime } = require("./agentcore-runtime-deploy");
      const result = await deployAgentCoreRuntime(manifest, { userEmail: req.auth?.email });
      return res.status(result.deployed || result.generated ? 200 : 422).json(result);
    }

    // Default: Bedrock Agents target
    const result = await deployAgentToAws(manifest, { userEmail: req.auth?.email });

    if (result.deployed || result.simulated) {
      const region = result.region || process.env.AWS_REGION || "us-east-1";
      // The Streamlit chat helper is local-dev only; in the cloud there is no localhost:8501.
      if (process.env.STREAMLIT_CHAT_ENABLED === "true") {
        try {
          const { launchStreamlitChat } = require("./streamlit-launcher");
          const agentId = result.agentId;
          const aliasId = result.aliasId || "live";
          const agentName = manifest.metadata.name;
          const chat = await launchStreamlitChat({ agentId, aliasId, region, agentName });
          result.chatUrl = chat.url;
          result.chatReused = chat.reused || false;
        } catch (err) {
          result.chatUrl = null;
          result.chatError = err.message;
        }
      } else {
        result.chatUrl = result.consoleUrl || (result.agentId
          ? `https://${region}.console.aws.amazon.com/bedrock/home?region=${region}#/agents/${result.agentId}`
          : null);
        result.chatIsConsole = true;
      }
    }

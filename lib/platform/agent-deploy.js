...
async function waitForAgentPrepared(client, agentId, maxAttempts = 40) {
  const { GetAgentCommand, PrepareAgentCommand } = require("@aws-sdk/client-bedrock-agent");
  for (let i = 0; i < maxAttempts; i++) {
    let got;
    try {
      got = await client.send(new GetAgentCommand({ agentId }));
    } catch (err) {
      // Fresh-region eventual consistency: the just-created agent isn't queryable yet.
      if (/ResourceNotFound|ThrottlingException|TooManyRequests/i.test(err.name + err.message)) {
        await sleep(3000);
        continue;
      }
      throw err;
...
async function deployAgentToAws(manifest, { userEmail } = {}) {
  ...
  try {
    ...
    const findAgentIdByName = async (name) => {
      let nextToken;
      do {

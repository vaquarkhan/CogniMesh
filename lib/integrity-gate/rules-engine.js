...
function requiresSecretArn(contract) {
  ...
  if (t === "rds" || t === "mysql") {
    // Skip check when Terraform will auto-provision Secrets Manager on deploy
    const provisioningMode = contract?.spec?.source?.provisioning?.mode;
    if (provisioningMode === "terraform") return [];
